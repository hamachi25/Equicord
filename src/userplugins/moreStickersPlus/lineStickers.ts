...
export function convertSticker(s: LineSticker): Sticker {
    return {
        ...
        image: s.animationUrl || s.popupUrl || s.staticUrl,
        title: s.id,
        stickerPackId: toStickerPackId(s.stickerPackId),
        isAnimated: !!s.animationUrl || !!s.popupUrl,
...
export function parseHtml(html: string): LineStickerPack {
    ...
    // 変更
    function extractStickers(selector: string): LineSticker[] {
    ...
    if (doc.querySelector('[data-test="sticker-item"]')) {
        stickers = extractStickers('[data-test="sticker-item"]');
    } else {
        stickers = extractStickers("[data-custom-api]");
    }
...
/**
  ...
  * @return {Promise<LineStickerPack>} The sticker pack.
  */
export async function getStickerPackById(id: string, region = "ja"): Promise<LineStickerPack> {
